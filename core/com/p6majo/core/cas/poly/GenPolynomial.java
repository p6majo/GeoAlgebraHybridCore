...
import com.p6majo.core.cas.exceptions.AlgebraicNotInvertibleException;
import com.p6majo.core.cas.exceptions.PreemptingException;
import com.p6majo.core.cas.kern.PrettyPrint;
import com.p6majo.core.cas.exceptions.NotInvertibleException;
import com.p6majo.core.cas.structure.RingElem;
import com.p6majo.core.cas.structure.UnaryFunctor;
...
public class GenPolynomial<C extends RingElem<C>>
                ...
                Iterable<Monomial<C>> {
                    ...
                    private static final Logger logger = new Logger(GenPolynomial.class);
    ...
    public void doPutToMap(ExpVector e, C c) {
            ...
            C a = val.get(e);
            if (a != null) {
                logger.log(Logger.Level.error,"map entry exists " + e + " to " + a + " new " + c);
            }
            hash = -1;
            blen = -1;
    ...
    public void doRemoveFromMap(ExpVector e, C c) {
        ...
        if (true) { //||debug
            ...
            if (!c.equals(b)) {
                logger.log(Logger.Level.error,"map entry wrong " + e + " to " + c + " old " + b);
    ...
    public void doPutToMap(SortedMap<ExpVector, C> vals) {
        for (Map.Entry<ExpVector, C> me : vals.entrySet()) {
            ExpVector e = me.getKey();
                C a = val.get(e);
                if (a != null) {
                    logger.log(Logger.Level.error,"map entry exists " + e + " to " + a + " new " + me.getValue());
                }
                hash = -1;
                blen = -1;
        ...
        for (Map.Entry<ExpVector, C> m : val.entrySet()) {
            ...
            if (u.equals(e)) {
                ...
                if (v.compareTo(uv) == 0) { // || v.negate().compareTo(uv) == 0
                    ...
                    if (ab * ab == a * b) { // cos == 1
                        fp.val.put(e, m.getValue());
                        logger.log(Logger.Level.info,"ab = " + ab + ", a = " + a + ", b = " + b + ", u = " + u + ", e = " + e
                                        + ", v = " + v);
                    }
                }
            }
    ...
    public GenPolynomial<C> divide(C s) {
        ...
        for (Map.Entry<ExpVector, C> m : val.entrySet()) {
            ...
            C c = c1.divide(s);
                C x = c1.remainder(s);
                if (!x.isZERO()) {
                    logger.log(Logger.Level.info,"divide x = " + x);
      ...
      if (S == null || S.isZERO()) {
        ...
        while (!r.isZERO()) {
            ...
            if (f.multipleOf(e)) {
                C a = r.leadingBaseCoefficient();
                f = f.subtract(e);
                //loger.log(Logger.Level.info,"red div = " + e);
                a = a.multiply(ci);
                h = S.multiply(a, f);
                r = r.subtract(h);
            } else {
    ...
    public GenPolynomial<C> map(final UnaryFunctor<? super C, C> f) {
        ...
        for (Monomial<C> m : this) {
            //loger.log(Logger.Level.info,"m = " + m);
            C c = f.eval(m.c);
            if (c != null && !c.isZERO()) {
                nv.put(m.e, c);
            }
        ...
        if (blen < 0L) {
            ...
            for (Monomial<C> m : this) {
                ...
                try { // hack
                    ...
                    logger.log(Logger.Level.error,"Exception, class: " + m.c.getClass());
    ...
    //private void writeObject(java.com.p6majo.io.ObjectOutputStream out) throws IOException {
