...
public final class TermOrder implements Serializable {
    ...
    Logger logger = new Logger(TermOrder.class);
    ...
    public TermOrder extend(int r, int k) {
        ...
        if (evord2 != 0) {
            logger.log(Logger.Level.warning,"TermOrder is already extended");
    ...
    public TermOrder contract(int k, int len) {
        ...
        if (evord2 == 0) {
                logger.log(Logger.Level.warning,"TermOrder is already contracted");
            return new TermOrder(evord);
        }
    ...
    public TermOrder reverse(boolean partial) {
        ...
        if (weight != null) {
            if (partial) {
                logger.log(Logger.Level.error,"partial reversed weight order not implemented");
            }
            long[][] w = new long[weight.length][];
            for (int i = 0; i < weight.length; i++) {
                long[] wi = weight[i];
            ...
            t = new TermOrder(w);
            logger.log(Logger.Level.info,"reverse = " + t + ", from = " + this);
            return t;
        }
        if (evord2 == 0) {
    ...
    public static int revert(int evord) {
        ...
        default: // REVITDEG, ITDEGLEX
            Logger.logging(Logger.Level.error,"can not revert " + evord);
            break;
        }
        return i;
    }
    ...
    public TermOrder permutation(List<Integer> P) {
        ...
        if (getEvord2() != 0) {
            ...
            logger.log(Logger.Level.warning,"split term order '" + this + "' not permutable, resetting to most base term order "
                            + tord);
        }
    ...
    public static TermOrder reverseWeight(long[][] w) {
        if (w == null) {
            Logger.logging(Logger.Level.warning,"null weight matrix ignored");
