...
public class BasicLinAlg<C extends com.p6majo.core.cas.structure.RingElem<C>> implements Serializable {


    private static final Logger logger = new Logger(BasicLinAlg.class);


    //private static final boolean debug = com.p6majo.logger.isDebugEnabled();
    ...
    public C scalarProduct(List<C> G, List<C> F) {
        ...
        if (it.hasNext() || jt.hasNext()) {
            logger.log(Logger.Level.error,"scalarProduct wrong sizes");
        }
        return sp;
    }
    ...
    public List<C> vectorAdd(List<C> a, List<C> b) {
        ...
        if (it.hasNext() || jt.hasNext()) {
            logger.log(Logger.Level.error,"vectorAdd wrong sizes");
        }
        return V;
    }
